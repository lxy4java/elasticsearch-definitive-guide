...
Elastticsearch的内置分词器都是全局可用的，不需要提前配置，((("language analyzers", "using")))它们也可以在字段图中直接指定某字段：
...
（英语）分词器，我们能更轻松的匹配而强化了记忆力，但却降低了精确排列源文档的能力。



为了获得两方面的优势，我们可以使用<<multi-fields,multifields>>（多字段）对`title`建立两次索引：
...
替换为该字段图，我们可以索引一些测试文档来展示怎么在搜索时使用两个字段：
